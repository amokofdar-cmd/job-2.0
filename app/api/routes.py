...
def seed_demo_data(db: Session = Depends(get_db)):
    ...
    if not profile:
        profile = CandidateProfile(
            ...
            master_resume="Python FastAPI SQL automation cloud APIs distributed systems",
            preferences_json='{"remote": true, "roles": ["backend", "platform"]}',
        )
        db.add(profile)
        db.commit()
    ...
    demo_jobs = [
        JobListing(
            ...
            description="Build Python APIs with SQL and cloud services",
        ),
        JobListing(
            source="ashby",
            company="Orbit",
            ...
            location="US",
            url="https://example.com/jobs/automation-1",
            description="Workflow automation, scripting, stakeholder collaboration",
        ),
    ]

    added = 0
    for job in demo_jobs:
        exists = db.execute(select(JobListing).where(JobListing.url == job.url)).scalar_one_or_none()
        if exists:
            continue
        db.add(job)
        added += 1
